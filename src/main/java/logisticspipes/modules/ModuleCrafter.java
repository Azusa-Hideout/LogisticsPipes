	...
	public void readFromNBT(@Nonnull NBTTagCompound tag) {
		...
		for (int i = 0; i < 9; i++) {
			...
			if (!advancedSatelliteUUIDArrayString.isEmpty()) {
				advancedSatelliteUUIDList.set(i, FastUUID.parseUUID(advancedSatelliteUUIDArrayString));
			}
		}
		...
		for (int i = 0; i < ItemUpgrade.MAX_LIQUID_CRAFTER; i++) {
			...
			if (!liquidSatelliteUUIDArrayString.isEmpty()) {
				liquidSatelliteUUIDList.set(i, FastUUID.parseUUID(liquidSatelliteUUIDArrayString));
			}
		}
